...
const App = () => {
    ...
    return user === null ? (<div>
            <Button variant={"contained"} onClick={() => authenticateWithGoogle()}>Sign In With Google</Button>
        {/*<div>{userId}</div>*/}
        {/*<div>Hello</div>*/}
        </div>) :
        ...
        (<div className="App">
            <Box>
                ...
                <Grid container spacing={2} className={"NoteList"}>
                    <NoteCard data={{text: user.uid}} key={0} deletefunc={fsDeleteData} />
                    {fsNotes.map((note) => (<NoteCard data={note} key={note.key} deletefunc={fsDeleteData}/>))}
                </Grid>
